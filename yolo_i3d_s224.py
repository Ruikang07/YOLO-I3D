...
from models.common_i3d import DetectMultiBackend

class YOLO_3D_Bottom(nn.Module):
    def __init__(self, device, weight_path):
        super(YOLO_3D_Bottom, self).__init__()      
        dnn = False,   # use OpenCV DNN for ONNX inference
        data = None    # yolo dataset.yaml path for training 
        fp16 = False   # use FP16 half-precision inference  
        self.yolo_2d_bottom = DetectMultiBackend(weight_path, device, dnn, data, fp16)
        self.pool = nn.MaxPool1d(2, 2)            
        #Freeze the parameters of yolo_2d_bottom
        for child in self.yolo_2d_bottom.modules():
            for param in child.parameters():
                param.requires_grad = False              
        
    def forward(self, x):#i3d input: x.shape = (B, ch, T, H, W)
...
class I3D(torch.nn.Module):
    def __init__(self,
        ...
        #self.avg_pool = torch.nn.AvgPool3d((2, 7, 7), (1, 1, 1)) # original
        self.avg_pool = torch.nn.AvgPool3d((2, 7, 7), (2, 1, 1)) # new
        ...
        #self.maxPool3d_2x1x1 = torch.nn.MaxPool3d(kernel_size=(2, 1, 1), stride=(2, 1, 1), ceil_mode=True)
        self.maxPool3d_2x1x1 = MaxPool3dTFPadding(kernel_size=(2, 1, 1), stride=(2, 1, 1), padding='SAME')        
        
    def forward(self, inp):
        out = inp
        out = self.maxPool3d_2x1x1(inp) #new added
