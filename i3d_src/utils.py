...
class Log(object):
    def __init__(self, dir='.', name=None):
        ...
        self.log_file = (dir+'/'+'log_i3d_{}.txt').format(time0)

    def __call__(self, *snippets, end=None):
        t1 = time.strftime("==> [%Y-%m-%d %H:%M:%S]", time.localtime()) 
        str1 = t1 + " " + "".join([str(s) for s in snippets])       
        if end is None:
            print(str1)
            with open(self.log_file, 'a') as f: print(str1, file=f)
        else:
            print(str1, end=end)
            with open(self.log_file, 'a') as f: print(str1, file=f)            
...
class Log_Loss(object):
    def __init__(self, dir='.', name=None):
        ...
        date0 = datetime.today().strftime('%Y-%m-%d')
        time0 = time.strftime("%Y-%m-%d-%H-%M-%S", time.localtime()) 
        self.log_file = (dir+'/'+'log_loss_{}.txt').format(time0)

    def __call__(self, *snippets, end=None):
        str1 = "".join([str(s) for s in snippets])       
        if end is None:
            print(str1)
            with open(self.log_file, 'a') as f: print(str1, file=f)
        else:
...
def build_data_path(is_image=False, data_root=None):
    ...
    for first_part in first_parts:
        ...
        if not temp_1.exists(): temp_1.mkdir(parents=True, exist_ok=True)
        for class_part in class_parts:
            # print(str(train_path/first_part/class_part))
            temp_2 = train_path / first_part / class_part
            if not temp_2.exists(): temp_2.mkdir(parents=True, exist_ok=True)
