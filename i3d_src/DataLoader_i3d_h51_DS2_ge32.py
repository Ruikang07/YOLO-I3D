...
def temporal_train_transform(images_list, sample_num=32):    
    frame_num = len(images_list) 
    if frame_num >= DownSample*sample_num:
        images_list = images_list[::DownSample]  
    frame_num = len(images_list)      
    if sample_num <= frame_num :
        start_frame = random.randint(0, (frame_num - sample_num))
        end_frame = start_frame + sample_num
        imgs_list = images_list[start_frame:end_frame]          
    else:
        len2 = sample_num - frame_num
        imgs_list = images_list + images_list[0:len2]  

    return imgs_list


def temporal_val_transform(images_list, sample_num=32):
    frame_num = len(images_list) 
    if frame_num >= DownSample*sample_num:
        images_list = images_list[::DownSample]  
    frame_num = len(images_list)   
