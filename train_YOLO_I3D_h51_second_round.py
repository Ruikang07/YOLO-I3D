...
def main(model_i3d, model_yolo_3d_bottom, \
    ...
    lr_list = [1.0e-4, 1.0e-5, 1.0e-6, 1.0e-7, 1.0e-8]
    lr = lr_list[0]
    lr_i = 0
    lr_i_max = 5
...
if __name__ == "__main__":    
    ...
    weight_i3d = "weights/yolo_i3d_top_s224_h51_d2_f16_first_round_val_acc_0.6747.pth"


    device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
    model_yolo_3d_bottom = YOLO_3D_Bottom(device, weight_yolo) 
    model_i3d = I3D(num_classes=Num_Classes, modality='rgb', dropout_prob=DropoutProb)
    model_i3d.to(device) 
